...
_build_single $board $shield $snippet $artifact cmake_args *west_args:
    #!/usr/bin/env bash
    set -euo pipefail
    export BOARD_ROOT="{{ justfile_directory() }}"
    export SHIELD_ROOT="{{ justfile_directory() }}"
...
draw: (draw-one "toucan") (draw-one "toucan36")

# parse and render SVG for one keyboard (name = toucan|toucan36)
draw-one name:
